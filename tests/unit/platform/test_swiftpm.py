...
"""Building a Swift package: for release, for this Mac, the chosen Xcode, and why when it does not build."""

from __future__ import annotations

from pathlib import Path

from sim_mirror.platform.swiftpm import BUILD_TIMEOUT_S, SwiftBuild, build_argv, build_package
...
def test_a_build_is_for_release_into_its_scratch_folder_for_this_mac() -> None:
    assert build_argv(Path("/src/helper"), Path("/state/build")) == (
...
async def test_a_package_that_builds_says_where_its_products_are() -> None:
    fake = FakeXcrun().on(
        ...
        then=lambda args: XcrunResult(
            0, "/state/build/arm64-apple-macosx/release\n" if args[-1] == "--show-bin-path" else "Compiling\n", ""
        ),
    )
    built = await build_package(Path("/src"), Path("/state/build"), developer_dir=XCODE_27, xcrun=fake)
    assert built == SwiftBuild(Path("/state/build/arm64-apple-macosx/release"))
