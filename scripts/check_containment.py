...
"""Fail if Xcode's tools or idb_companion are named anywhere they could be started from.

SimMirror runs real programs: ``xcrun`` (simctl, xcodebuild, xcresulttool) boots devices, installs apps and builds
whatever project an agent points it at, and ``idb_companion`` streams a device's screen and takes its touches. The
...
def main() -> int:
    found = offenders()
    if not found:
        print("containment ok: Xcode's tools and idb_companion are named only by the modules that run them")
        return 0
    print("an Xcode tool or idb_companion is named outside the module that runs it:\n", file=sys.stderr)
