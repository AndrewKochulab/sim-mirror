...
- **The agent's cursor stays while the agent works.** It used to leave five seconds after each gesture, so it blinked
  ...
  tool call sends a new `working` agent event when it starts, every ten seconds while it runs and when it ends, and every
  agent event carries `linger_ms`; a viewer from before ignores both. The cursor is drawn by the viewer over the
