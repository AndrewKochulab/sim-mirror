# Compatibility

<!-- Generated by scripts/gen_docs.py -- do not edit. Run `make generate` after changing its source. -->

Where SimMirror is expected to work, and where it has been checked. A row says **verified**, with the date, only when it was checked end to end on real hardware; **expected** means it should work and nobody has checked it yet. Checked it yourself? Send a [compatibility report](https://github.com/AndrewKochulab/sim-mirror/issues/new?template=compatibility-report.yml) with `sim-mirror doctor --json`.

## macOS

SimMirror needs a Mac with a logged-in desktop session: simulators do not show over SSH or from a launch daemon.

| Version | Status | Notes |
|---|---|---|
| macOS 26 | **verified 2026-09-16** | Checked end to end on macOS 26.6.2. Xcode 26 needs macOS 26.2 or later, Xcode 27 macOS 26.6 or later. |
| macOS 15 | **expected** | With Xcode 16.4: the simctl connector only (view-only), since idb_companion 1.5 needs Xcode 26. |
| Intel Macs | **unsupported** | idb_companion 1.5 is built for Apple Silicon, and Xcode 27 runs only on Apple Silicon. |

## Xcode

| Version | Status | Notes |
|---|---|---|
| Xcode 26 | **verified 2026-09-16** | Checked end to end with Xcode 26.6 (17F113) and the iOS 18.6 and 26.5 runtimes, on an iPhone 17 Pro simulator. |
| Xcode 27 | **verified 2026-09-16** | Checked with Xcode 27.0 (27A266a) and the iOS 27.0 runtime on an iPhone 18 Pro, beside Xcode 26.6 and chosen both by xcode-select and by device.developer_dir: the live screen in Chrome, touches, named keys, Home, the element tree, the agent tools and the doctor's tap. Text is typed as key presses (#27): an agent's type step and the viewer's typing arrived on iOS 27.0 and 26.5 alike, and a paste, which iOS 27.0 refuses, is left for text no key types. SimulatorKit moved to Contents/SharedFrameworks. With Device Hub open, a device booted then still took idb's taps; one Device Hub has taken over may not (not checked). |
| Xcode 16.4 and earlier | **expected** | The simctl connector only (view-only). |

## Connectors

`auto` uses the native helper, then idb where idb_companion is installed, then simctl, saying why it fell back. See Connectors.

| Connector | Status | Notes |
|---|---|---|
| native (SimMirror's helper) | **verified 2026-09-17** | Checked on iOS 26.5 with Xcode 26.6 and iOS 27.0 with Xcode 27.0 chosen by device.developer_dir (CoreSimulator 1171.7, dtuhid input): the H.264 stream in Chrome on both and JPEG on iOS 26.5, taps, drag scrolling, typing and Home in the viewer, and an agent's snapshots, screenshots, taps by ref and Home. Switching connectors in the settings panel moved the device between native, idb and simctl at once; with connectors.native.helper_path pointed at nothing, native named refused with the reason and auto used idb, then moved back to native once the helper could run. The doctor reached a booted device through the helper and its test tap went through native. The live suite passed on both with the universal build, and benchmarks/connector_latency.py measured it against idb and simctl (see Connectors). Indigo input, for CoreSimulator before 1155.4, was not checked. |
| idb (idb_companion 1.5) | **verified 2026-09-16** | Full control, all checked on a device: H.264 and JPEG streams, touch, drag, wheel, buttons, appearance, typing, paste and the element tree. |
| simctl | **verified 2026-09-16** | View-only, checked on a device with the companion hidden: JPEG at up to 4 frames a second, screenshots, appearance, install, launch, open URL, logs. No input and no element tree, and the tools that need them are refused. |
| mcpbridge (Xcode 27.0) | **verified 2026-09-16** | Checked on an iOS 27.0 simulator with Xcode 27.0 (27A266a) named by device.developer_dir while xcode-select named Xcode 26.6: the screen in Chrome, view-only; Claude Code's snapshots of Safari through Xcode's UI hierarchy; the doctor's read; sim-mirror xcode approve. Merged into idb's snapshots (connectors.mcpbridge.merge), Claude Code saw example.com's link and tapped it through idb. Xcode's session was ended a minute after the last read and when the daemon stopped. |
| App SDK (SimMirrorKit) | **verified 2026-09-17** | Checked with examples/app-sdk on iOS 26.5 with Xcode 26.6, idb driving the device, through the agent routes: built and launched with sim_build_run, its UIKit form, SwiftUI and tagged screens, an alert and the keyboard read, named and added to, a tap on a card only the app's hierarchy has, merging and naming switched off and on, the viewer's chip in Chrome, the doctor's check and sim-mirror app hierarchy. SwiftUI's debug data read on iOS 26.5. Requests without the secret, from a web page or to another host were refused; a Release build wrote no listing and opened no port. The package's and the app's tests passed on Xcode 26.6 (iOS 26.5 and 18.6) and 27.0 (iOS 27.0). |

## Build and test

`sim_build_run` and `sim_test`, driven by Claude Code on a probe project with XCTest and Swift Testing suites, a scheme named `Probe (Staging)` and a test target that does not compile.

| Xcode | Status | Notes |
|---|---|---|
| Xcode 26.6 | **verified 2026-09-16** | On an iOS 26.5 simulator: failures at their project paths, each failing test run again alone by the id the answer gave, the compile error of tests that do not build, and a build in the Staging configuration installed and launched. |
| Xcode 27.0 | **verified 2026-09-16** | On an iOS 27.0 simulator, and a run sent to an iOS 26.5 one with `destination`: the same answers, with a failure's place read from the `sourceLocation` Xcode 27 gives, and a refused configuration answered with the ones the project has. |

## MCP clients

| Client | Status | Notes |
|---|---|---|
| Claude Code in a terminal, with the plugin | **expected** | `/plugin install sim-mirror@sim-mirror`. The plugin manifest validates; the tools themselves were checked through the same server started by hand. |
| Claude Code in a terminal, with `claude mcp add` | **verified 2026-09-16** | A session drove a real device through the tools: snapshots and taps, no screenshots, its name on the cursor in the viewer. |
| OpenAI Codex | **expected** | Raise `tool_timeout_sec` for build tools. |
| Cursor | **expected** |  |
| Other MCP clients over stdio | **expected** | Any client that starts a stdio server and lists its tools. |

## Browsers

H.264 is decoded with WebCodecs, which needs a secure page: http://127.0.0.1 and http://localhost qualify. Anywhere H.264 cannot be decoded, the viewer uses JPEG.

| Browser | Status | Notes |
|---|---|---|
| Chrome | **verified 2026-09-16** | Checked on Chrome: H.264 decoded in the page, a person's input, the agent cursor, an iframe and the web component. |
| Safari | **verified 2026-09-17** | Checked on Safari 26.6.2 (macOS 26.6.2) with an iPhone 17 Pro simulator on iOS 26.5: H.264 decoded in the page (the daemon logged the viewer decoding h264 and jpeg, and streamed h264 with no fall back to JPEG), a person's tap opened Settings, and an agent's tap on General was drawn as the agent cursor before it landed. The iframe and the web component were not checked in Safari. |
| Firefox | **expected** |  |
| Edge | **expected** |  |
