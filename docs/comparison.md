...
|---|---|---|---|---|---|---|---|
| **SimMirror** (this project) | Apache-2.0 | MCP server, CLI, local daemon, web component, Python library, Claude Code plugin | Yes: H.264 through WebCodecs, or JPEG | Yes: idb_companion's accessibility tree, as snapshots with refs and diffs | Yes: idb_companion HID; view-only fallback over simctl | iOS simulator | v0.1.0 (preview) |
